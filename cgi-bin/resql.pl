...
if ( $q->request_method() =~ /^POST$/ && $q->path_info =~ /^\/login\/?/ ) {
	...
	print $q->header( {cookie => [$cookie], "content-type" => "application/json"} );
	...
	if ( my $user = $stbc->fetchrow_hashref ) { # is logged in: sessionid cookie verified
		...
		if ( $q->request_method() =~ /^GET$/ ) {
			...
			if ( $q->path_info =~ /^\/([a-zA-Z]+)\/([a-zA-Z0-9]+)$/ ) {
				...
				if ( $user->{Role_ID} == 1 && ( $table =~ /.+Benutzer.*/ || $table =~ /^Benutzer.+/ ) ) {
					
					my @values = ($id, $user->{ID});
					my $sql = "DELETE FROM `$table` WHERE `ID` = ? AND `Benutzer_ID` = ?";
					eval {
			...
			print encode_json({result => 0, reason => "supported request methods: GET and POST."});
		}
