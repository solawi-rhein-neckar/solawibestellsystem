    	...
    	if (path == 'Benutzer' && result && result.type == 'insert' && result.id && modules) {
    		...
    		for (var i = 0; i < modules.length; i++) {
    			if (modules[i] && modules[i].ID && (modules[i].AnzahlProAnteil || modules[i].ID == 2)) {
    	    		...
    	    		if (anteile) {
    					postAjax('BenutzerModulAbo', {Benutzer_ID: userId, Modul_ID: modules[i].ID, Anzahl: anteile*(modules[i].AnzahlProAnteil), StartWoche: data.PunkteWoche ? data.PunkteWoche : sbs.selectedWeek, EndWoche: '9999.99', BezahltesModul: modules[i].ID == 4}, createReloadFunction(userId));
    				}
    			}
    		}
