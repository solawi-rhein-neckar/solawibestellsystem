...
SET @query = CONCAT('
	SELECT Woche as `00.Woche`,
		   SUM( IF(Produkt = \'Milch, 0.5L\', cast(Anzahl/2 as decimal(5,1)), 0) ) AS `06.Milch`,',
		   @query, ',
		   SUM(IF(NOT (Produkt LIKE \'Gemuese\'),0, Urlaub)) as `99.Urlauber`,
		  ...
		  SUM(IF((NOT (Produkt LIKE \'Gemuese\')) OR BenutzerId <> (SELECT Min(ID) FROM Benutzer Where Benutzer.Depot_ID = subq.Depot_ID),0, (SELECT Sum(FleischAnteile) FROM Benutzer where Benutzer.Depot_ID = `subq`.`Depot_ID`))) as `98.FleischAnteileErlaubt`,
...
CALL BenutzerBestellungGesamt(NULL, FALSE);
